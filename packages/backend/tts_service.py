...
class TTSService:
    ...
    def _get_from_cache_validated(self, cache_key: str, text: str) -> tuple[Optional[bytes], bool]:
        ...
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT * FROM get_tts_cache_entry_validated(%s, %s)", (cache_key, text))
                result = cur.fetchone()
    ...
    def _save_to_cache_validated(self, cache_key: str, audio_content: bytes, text: str, language: str) -> bool:
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT save_tts_cache_entry_validated(%s, %s, %s, %s, %s)",
    ...
    def synthesize_speech(self, text: str, language: str) -> Optional[bytes]:
        ...
        try:
            ...
            if not saved:
                logger.error(f"Failed to cache TTS - validation failed during save")
