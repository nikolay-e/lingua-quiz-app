...
export async function fetchWordSets(token, wordListName) {
  ...
  try {
    ...
    if (token) {
      ...
      try {
        ...
        if (tokenParts.length === 3) {
          ...
          console.debug(`[dataHandler] Token expiry: ${expiry}, user ID: ${payload.userId || 'none'}`);
        }
      } catch (e) {
        console.debug(`[dataHandler] Token doesn't appear to be a valid JWT: ${e.message}`);
      }
    } else {
    ...
    console.debug(`[dataHandler] WordListName: "${wordListName}" (trimmed: "${trimmedWordListName}", encoded: "${encodedWordListName}")`);
    ...
    console.debug(`[dataHandler] Making fetch request with token: ${token ? 'present' : 'missing'}`);
    ...
    if (!response.ok) {
      ...
      try {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          const errorMessage = errorData.message || `Failed to fetch word sets (Status: ${response.status})`;
          console.error(`[dataHandler] API error: ${errorMessage}`, errorData);
          throw new Error(errorMessage);
        } else {
          // If not JSON, try to get the text
          const textResponse = await response.text();
          console.error(`[dataHandler] API error (${response.status}): Non-JSON response:`, textResponse);
        ...
        console.error(`[dataHandler] Original HTTP error: ${response.status} ${response.statusText}`);
        throw new Error(`Failed to fetch word sets (Status: ${response.status}): ${parseError.message}`);
      }
    }

    let wordSetData = await response.json();
    console.debug(`[dataHandler] Successfully fetched word sets. Length: ${wordSetData.length}, First item sample:`, 
    ...
    if (!wordSetData) {
      console.error(`[dataHandler] wordSetData is null or undefined. This should never happen with a 200 OK response.`);
    ...
    if (!Array.isArray(wordSetData)) {
      ...
      errorHandler.showError('Invalid response format from server. Expected an array of word pairs.');
      return null;
    }
    ...
    const validItems = wordSetData.filter((item, index) => {
      ...
      const isValid = item && 
        typeof item === 'object' && 
        item.wordPairId !== null && item.wordPairId !== undefined &&
        item.sourceWord && typeof item.sourceWord === 'string' && 
        item.targetWord && typeof item.targetWord === 'string';
    ...
    if (invalidItems.length > 0) {
      console.error(`[dataHandler] Found ${invalidItems.length} invalid items:`, 
        JSON.stringify(invalidItems.slice(0, 3)));
    }
    
    if (validItems.length === 0) {
      console.error(`[dataHandler] No valid word pairs found in data. Returning null.`);
      errorHandler.showError('No valid word pairs found for this quiz. Please select another quiz.');
      return null;
    }
    
    // If some items were filtered out, warn but continue with valid items
    if (validItems.length < wordSetData.length) {
      console.warn(`[dataHandler] Filtered out ${wordSetData.length - validItems.length} invalid word pairs`);
...
export async function fetchWordLists(token) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const errorMessage = errorData.message || `Failed to fetch word lists (Status: ${response.status})`;
      console.error(`[dataHandler] API error fetching word lists: ${errorMessage}`);
      throw new Error(errorMessage);
    }

    const data = await response.json();
    console.debug(`[dataHandler] Successfully fetched word lists. Count: ${data.length}, Lists:`, 
