...
import { STATUS, DIRECTION, CORRECT_ANSWERS_TO_MASTER, MAX_MISTAKES_BEFORE_DEGRADATION } from '../../src/js/constants.js';
...
describe('Quiz Flow Integration', () => {
  ...
  describe('Complete Quiz Flow Integration', () => {
    it('should follow the complete question-answer-feedback-stats flow', async () => {
      ...
      const correctAnswer = direction === DIRECTION.NORMAL 
        ? currentPair.targetWord 
        : currentPair.sourceWord;
      ...
      expect(appInstance.statsManager.stats.attemptsPerTranslationIdAndDirection[directionKey].attempts).toBe(1);
      expect(appInstance.statsManager.stats.attemptsPerTranslationIdAndDirection[directionKey].correct).toBe(1);
  ...
  describe('Statistics Integration', () => {
    it('should track complete statistics across answer submissions', async () => {
      ...
      const wordStats = appInstance.statsManager.stats.attemptsPerTranslationIdAndDirection[directionKey];
      ...
      expect(appInstance.statsManager.stats.incorrectPerTranslationIdAndDirection[directionKey]).toBe(1);
      ...
      const reverseStats = appInstance.statsManager.stats.attemptsPerTranslationIdAndDirection[reverseKey];
