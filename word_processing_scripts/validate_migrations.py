...
from migration_utils import (
    calculate_migration_ids,
    extract_data_from_file,
    get_language_base_offsets,
...
class MigrationValidator:
    ...
    def validate_id_consistency(
        self, all_data: List[Tuple], file_data_map: Dict[str, List[Tuple]]
    ) -> None:
        ...
        for file_path, file_entries in file_data_map.items():
            ...
            for i in range(1, len(sorted_entries)):
                ...
                prev_wpid, prev_sid, prev_tid = (
                    prev_entry[0],
                    prev_entry[1],
                    prev_entry[2],
                )
                curr_wpid, curr_sid, curr_tid = (
                    curr_entry[0],
                    curr_entry[1],
                    curr_entry[2],
                )
                ...
                if curr_wpid != prev_wpid + 1:
                    ...
                    for missing_wpid in range(prev_wpid + 1, curr_wpid):
                        ...
                        self.file_missing_entries[file_path].append(
                            (prev_wpid, missing_entry)
                        )
        ...
        for range_key in id1_ranges:
            ...
            for i in range(1, len(ids)):
                if ids[i] != ids[i - 1] + 1:
                    ...
                    if gap_size > 0:
                        gaps.append(
                            f"Range {range_key}: gap of {gap_size} between {ids[i-1]} and {ids[i]}"
                        )
    ...
    def validate_uniqueness(self, all_data: List[Tuple]) -> None:
        ...
        for i, (
            ...
            translation_id_usage[translation_id].append(
                (
                    i,
                    word_pair_id,
                    source_word_id,
            ...
            word_pair_id_usage[word_pair_id].append(
                (
                    i,
                    word_pair_id,
                    source_word_id,
            ...
            source_word_id_usage[source_word_id].append(
                (
                    i,
                    word_pair_id,
                    source_word_id,
        ...
        for lang_range, count in missing_entries_count.items():
            if count > 0:
                self.warnings.append(
                    f"Range {lang_range}xxx has {count} missing entries that need to be filled (marked with placeholder 'word')"
                )
        ...
        translation_duplicates = {
            tid: entries
            for tid, entries in translation_id_usage.items()
            if len(entries) > 1
        }
            ...
            for translation_id, entries in sorted(translation_duplicates.items()):
                ...
                for entry_idx, (
                    ...
                    if base_offset:
                        ...
                        if validate_migration_ids(
                            wpid, swid, tid, base_offset, sequence_number
                        ):
                ...
                for entry_idx, (
                    ...
                    print(
                        f"    Line {line_idx+1}: ({wpid}, {swid}, {tid}, '{word}', '{trans}', ...)"
                    )

        # Word pair ID duplicates
        word_pair_duplicates = {
            wpid: entries
            for wpid, entries in word_pair_id_usage.items()
            if len(entries) > 1
        }
        ...
        source_word_duplicates = {
            swid: entries
            for swid, entries in source_word_id_usage.items()
            if len(entries) > 1
        }
        if source_word_duplicates:
            ...
            for source_word_id, entries in sorted(source_word_duplicates.items()):
                if len(entries) > 1:
                    print(
                        f"  Source Word ID {source_word_id} used {len(entries)} times:"
                    )
                    for i, (
                        line_idx,
                        wpid,
        ...
        if translation_duplicates:
            self.errors.append(
                f"Found {len(translation_duplicates)} translation ID duplicates (see detailed analysis above)"
            )
        if word_pair_duplicates:
            self.errors.append(
                f"Found {len(word_pair_duplicates)} word pair ID duplicates (see detailed analysis above)"
            )
        source_dups_count = len(
            [x for x in source_word_duplicates.values() if len(x) > 1]
        )
        if source_dups_count > 0:
            self.errors.append(
                f"Found {source_dups_count} source word ID duplicates (see detailed analysis above)"
            )
        ...
        for range_key, entries in by_range.items():
            ...
            for word, translations in word_to_translations.items():
                if len(translations) > 1:
                    self.warnings.append(
                        f"Word '{word}' in range {range_key} has multiple translations: {list(translations)}"
                    )
            ...
            for translation, words in translation_to_words.items():
                if len(words) > 1:
                    self.warnings.append(
                        f"Translation '{translation}' in range {range_key} maps to multiple words: {list(words)}"
                    )
    ...
    def validate_data_integrity(self, all_data: List[Tuple]) -> None:
        ...
        for (
            ...
            if not translation.strip():
                self.errors.append(
                    f"Empty translation field for word_pair_id {word_pair_id}"
                )

            if not example.strip():
                self.warnings.append(
                    f"Empty example field for word_pair_id {word_pair_id}"
                )

            if not example_translation.strip():
                self.warnings.append(
                    f"Empty example translation field for word_pair_id {word_pair_id}"
                )

            # Check that word IDs are positive
            if source_word_id <= 0:
                self.errors.append(
                    f"Invalid source_word_id {source_word_id} for word_pair_id {word_pair_id}"
                )

            if target_word_id <= 0:
                self.errors.append(
                    f"Invalid target_word_id {target_word_id} for word_pair_id {word_pair_id}"
                )
    ...
    def validate_ranges(self, all_data: List[Tuple]) -> None:
        ...
        for range_key, stats in sorted(range_stats.items()):
            print(
                f"  Range {range_key}xxx: {stats['count']} entries, IDs {stats['min']}-{stats['max']}"
            )
    ...
    def analyze_stem_coverage(self, all_data: List[Tuple]) -> None:
        ...
        try:
            ...
            essential_spanish = [
                word for word in raw_spanish if is_essential_word(word)
            ]

            print(
                f"  Essential vocabulary: German {len(essential_german)} words, Spanish {len(essential_spanish)} words"
            )
        ...
        for word in essential_german:
            ...
            for base in possible_bases:
                if (
                    base in german_stems_in_migration
                    or stem in german_stems_in_migration
                    or any(
                        stem in existing_stem
                        for existing_stem in german_stems_in_migration
                    )
                    or any(
                        base in existing_stem
                        for existing_stem in german_stems_in_migration
                    )
                ):
                    found = True
        ...
        print(
            f"  Coverage: {german_covered}/{len(essential_german)} ({coverage_percent:.1f}%)"
        )
        ...
        if german_missing_concepts:
            ...
            if articles_pronouns:
                print(
                    f"    🔸 Articles/Pronouns ({len(articles_pronouns)}): {', '.join(articles_pronouns)}"
                )
            if prepositions:
                print(
                    f"    🔸 Prepositions ({len(prepositions)}): {', '.join(prepositions)}"
                )
            if conjunctions:
                print(
                    f"    🔸 Conjunctions ({len(conjunctions)}): {', '.join(conjunctions)}"
                )
            if adverbs:
                print(
                    f"    🔸 Adverbs/Particles ({len(adverbs)}): {', '.join(adverbs)}"
                )
            if adjectives:
        ...
        for word in essential_spanish:
            ...
            for base in possible_bases:
                if (
                    base in spanish_stems_in_migration
                    or stem in spanish_stems_in_migration
                    or any(
                        stem in existing_stem
                        for existing_stem in spanish_stems_in_migration
                    )
                    or any(
                        base in existing_stem
                        for existing_stem in spanish_stems_in_migration
        ...
        print(
            f"  Coverage: {spanish_covered}/{len(essential_spanish)} ({coverage_percent:.1f}%)"
        )
    ...
    def validate_top1k_coverage(self, all_data: List[Tuple]) -> None:
        ...
        try:
            ...
            def is_valid_word(word):
                ...
                if len(word) == 1 and word not in [
                    "i"
                ]:  # 'i' might be valid in some contexts
                    return False
            ...
            top_1000_german = set(
                self.normalize_word(word, is_german=True)
                for word in raw_german
                if is_valid_word(word)
            )
            top_1000_spanish = set(
                self.normalize_word(word, is_german=False)
                for word in raw_spanish
                if is_valid_word(word)
            )

            print(
                f"  Filtered top 1k: German {len(top_1000_german)} words, Spanish {len(top_1000_spanish)} words"
            )
        ...
        for id1, id2, id3, word, translation, example, example_translation in all_data:
            if str(id1).startswith("3"):  # German range
                ...
                if "|" in word:
                    for alt in word.split("|"):
                        german_words_in_migration.add(
                            self.normalize_word(alt.strip(), is_german=True)
                        )
                else:
                    german_words_in_migration.add(
                        self.normalize_word(word, is_german=True)
                ...
                if "|" in word:
                    for alt in word.split("|"):
                        spanish_words_in_migration.add(
                            self.normalize_word(alt.strip(), is_german=False)
                        )
                else:
                    spanish_words_in_migration.add(
                        self.normalize_word(word, is_german=False)
        ...
        print("\n📚 German (Range 3xxx):")
        print(
            "  Note: Top 1k includes inflected forms (e.g., bin, bist, ist, sind from 'sein')"
        )
        print(
            "        Migration uses dictionary/base forms (e.g., 'sein' not 'bin/bist/ist')"
        )
        ...
        if missing_german:
            ...
            print(
                f"    - {len(verb_forms)} verb conjugations (e.g., {', '.join(verb_forms[:5])}...)"
            )
            print(f"    - {len(articles_pronouns)} articles/pronouns inflected forms")
            print(
                f"    - {len(proper_nouns)} proper nouns (cities, names, organizations)"
            )
            ...
            self.warnings.append(
                f"Missing {len(missing_german)} words from German top 1k"
            )
        else:
        ...
        extra_german = (
            german_words_in_migration - top_1000_german - {"word"}
        )  # Exclude placeholder
        if extra_german:
            print(
                f"  📊 Found {len(extra_german)} German words in migration that are NOT in top 1k"
            )
            print(f"    {', '.join(sorted(extra_german))}")
            self.warnings.append(
                f"Found {len(extra_german)} German words not in top 1k list"
            )
        ...
        if missing_spanish:
            ...
            self.warnings.append(
                f"Missing {len(missing_spanish)} words from Spanish top 1k"
            )
        else:
        ...
        extra_spanish = (
            spanish_words_in_migration - top_1000_spanish - {"word"}
        )  # Exclude placeholder
        if extra_spanish:
            print(
                f"  📊 Found {len(extra_spanish)} Spanish words in migration that are NOT in top 1k"
            )
            print(f"    {', '.join(sorted(extra_spanish))}")
            self.warnings.append(
                f"Found {len(extra_spanish)} Spanish words not in top 1k list"
            )
            ...
            for prev_id, missing_entry in reversed(missing_entries):
                ...
                for i, line in enumerate(lines):
                    if f"({prev_id}," in line and not line.strip().startswith("--"):
                        ...
                        if (
                            i + 1 < len(lines)
                            and f"-- MISSING: {missing_entry}" in lines[i + 1]
                        ):
