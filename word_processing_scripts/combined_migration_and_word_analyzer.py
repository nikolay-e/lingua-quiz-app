...
class EnhancedAnalyzer:
        ...
        if success:
            ...
            if show_details:
                ...
                if validator.warnings:
                    ...
                    for i, warning in enumerate(validator.warnings, 1):  # Show all warnings
                        print(f"  {i}. {warning}")
    ...
    def analyze_language(self, language: str, top_n: int = 1000, show_details: bool = True) -> List[Tuple[str, float, str, str]]:
    ...
    def run_complete_analysis(self, languages: List[str] = None, top_n: int = 1000, validate_first: bool = True) -> Dict[str, List]:
        ...
        for language in languages:
            try:
                recommendations = self.analyze_language(language, top_n=top_n, show_details=True)
    ...
    def _print_summary(self, results: Dict[str, List]):
        ...
        for language, recommendations in results.items():
            lang_name = {'de': 'German', 'en': 'English', 'es': 'Spanish'}.get(language, language)
        ...
        if self.validation_results:
            print(f"🔧 Migration validation: {len(self.validation_results['errors'])} errors, {len(self.validation_results['warnings'])} warnings")
...
def main():
    ...
    parser.add_argument('--languages', nargs='+', choices=['de', 'en', 'es'],
                       default=['de', 'en', 'es'],
