    ...
    def analyze_word(self, word: str, existing_words: Set[str]) -> Tuple[str, str, str]:
        ...
        if "'" in word and pos_tag != "NOUN":
            if lemma in existing_words:
                return (
                    "inflected_form",
                    pos_tag,
                    f"Contraction of '{lemma}' (e.g., it's, don't)",
                )
        ...
        if pos_tag in ["PUNCT", "SYM", "SPACE", "X"]:
            return (
                "symbol_or_other",
                pos_tag,
                "Symbol, punctuation, or non-linguistic token",
            )

        if token.is_stop or pos_tag in [
            "ADP",
            "AUX",
            "CCONJ",
            "SCONJ",
        ...
        if pos_tag in self.pos_thresholds:
            if freq >= self.pos_thresholds[pos_tag]:
                if pos_tag in self.category_mapping:
                    return (
                        self.category_mapping[pos_tag],
                        pos_tag,
                        f"High-frequency {pos_tag}",
                    )
            else:
                return (
                    "low_frequency",
                    pos_tag,
                    f"Too infrequent for learners (freq: {freq:.6f})",
                )
