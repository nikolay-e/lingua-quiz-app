...
def get_language_base_offsets() -> dict:
    ...
    return {"de": 3000000, "es": 4000000, "en": 8000000}


def calculate_migration_ids(
    base_offset: int, sequence_number: int
) -> Tuple[int, int, int]:
    """
...
def validate_migration_ids(
    ...
    """
    expected_word_pair_id, expected_source_id, expected_target_id = (
        calculate_migration_ids(base_offset, sequence_number)
    )
    return (
        word_pair_id == expected_word_pair_id
...
def print_category_results(
    categories: Dict,
    category_info: List[Tuple[str, str]],
    title: str,
    ...
    for cat_name, cat_display in category_info:
        if cat_name in categories and categories[cat_name]:
            ...
            words_to_show = (
                ...
                else categories[cat_name][:max_examples]
            )
...
def print_ignore_results(
    categories: Dict,
    ignore_info: List[Tuple[str, str, str]],
    title: str,
    ...
    for cat_name, cat_display, explanation in ignore_info:
        if cat_name in categories and categories[cat_name]:
            ...
            if show_examples and cat_name in [
                "inflected_form",
                "morphological_variant",
            ]:
...
def print_final_recommendations(
    recommendations: List[Tuple[str, float, str, str]], max_display: int = None
) -> None:
...
def print_analysis_summary(
    total_to_add: int,
    total_not_to_add: int,
    recommendations: List,
    analysis_method: str = "NLP",
) -> None:
...
def display_standard_results(
    ...
    total_to_add = print_category_results(
        categories, essential_info, "✅ WORDS TO ADD", max_examples=None
    )
    total_not_to_add = print_ignore_results(
        categories, get_ignore_categories(), "❌ WORDS TO IGNORE"
    )

    print_final_recommendations(recommendations)
    print_analysis_summary(
        total_to_add, total_not_to_add, recommendations, analysis_method
    )
