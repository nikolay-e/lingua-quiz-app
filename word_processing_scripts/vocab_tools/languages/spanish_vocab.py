...
class SpanishVocabularyAnalyzer(VocabularyAnalyzer):
    ...
    def __init__(self, migrations_directory: Path = None, silent: bool = False):
        ...
        if not silent:
            print(f"🇪🇸 Initializing Spanish vocabulary analyzer...")
        ...
        self.regular_verb_patterns = {
            'ar': ['o', 'as', 'a', 'amos', 'áis', 'an', 'aba', 'abas', 'ábamos', 'aban', 'é', 'aste', 'ó', 'amos', 'aron'],
            'er': ['o', 'es', 'e', 'emos', 'éis', 'en', 'ía', 'ías', 'íamos', 'ían', 'í', 'iste', 'ió', 'imos', 'ieron'],
            'ir': ['o', 'es', 'e', 'imos', 'ís', 'en', 'ía', 'ías', 'íamos', 'ían', 'í', 'iste', 'ió', 'imos', 'ieron'],
        }
    ...
    def analyze_word_linguistics(self, word: str, existing_words: Set[str], rank: int = None) -> Tuple[str, str, str]:
        ...
        regular_verb_base = self._find_regular_verb_base(normalized_word, existing_words)
        if regular_verb_base:
            return "inflected_forms", "VERB", f"Conjugated form of '{regular_verb_base}'"
